...
import {
  ...
  Settings as SettingsIcon, Menu, X, Orbit,
} from 'lucide-react';
import { ViewId, Client, Service, Quote, Receipt as ReceiptType, Settings } from './types';
import { getClients, saveClients, getServices, saveServices, getQuotes, saveQuotes, getReceipts, saveReceipts, getSettings, saveSettings } from './store';
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
        ...
        <header className="flex items-center justify-between px-4 lg:px-6 py-4 border-b border-slate-200 bg-white shrink-0">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 hover:bg-slate-100 rounded-lg lg:hidden transition"
            >
              <Menu className="w-5 h-5 text-slate-600" />
            </button>
            <div className="flex items-center gap-2">
              <activeNav.Icon className="w-5 h-5 text-blue-600" />
              <h2 className="text-lg font-bold text-slate-800">{activeNav.label}</h2>
            </div>
          </div>
          <p className="text-sm text-slate-400 hidden sm:block">{settings.businessName}</p>
        </header>

        {/* Content */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 scrollbar-thin bg-slate-50">
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-blue-600 hover:bg-blue-500 rounded-full shadow-2xl shadow-blue-500/30 flex items-center justify-center lg:hidden transition-all active:scale-95"
      >
