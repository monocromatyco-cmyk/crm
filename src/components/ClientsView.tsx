...
import { Plus, Search, Edit2, Trash2, X, User, Building2, Phone, Mail, MapPin } from 'lucide-react';
...
const empty: Omit<Client, 'id' | 'createdAt'> = {
  companyName: '', representativeName: '', role: '', fullName: '',
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filtered.map((c) => (
            <div key={c.id} className="bg-white border border-slate-200 rounded-xl p-4 flex items-start gap-4 shadow-sm">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-800">{isNew ? 'Nuevo cliente' : 'Editar cliente'}</h2>
            ...
            <div className="space-y-4">
              ...
              <Field label="Empresa" icon={<Building2 className="w-4 h-4" />}>
                <input
