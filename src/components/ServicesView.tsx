    ...
    <div className="space-y-4 view-enter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filtered.map((s) => (
            <div
              key={s.id}
              onClick={() => openEdit(s)}
              className="card-base p-4 hover:border-blue-300 transition-all duration-200 cursor-pointer flex items-center justify-between"
            >
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-800 truncate">{s.name}</p>
                {s.description && <p className="text-xs text-gray-500 truncate mt-0.5">{s.description}</p>}
                <p className="text-xs text-gray-500 mt-1">{s.unit} · <span className="text-blue-600 font-semibold">{fmt(s.basePrice)}</span></p>
              </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-start justify-center overflow-y-auto p-4">
          <div className="card-base w-full max-w-md my-8 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">{isNew ? 'Nuevo servicio' : 'Editar servicio'}</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs text-gray-500 font-medium mb-1 block">Precio base</label>
